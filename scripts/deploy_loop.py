...
def announce_reset(kind: str, auto: bool) -> dict:
    """Tell the operator what state this cycle needs, and wait for it."""
    insts = vs.instances()
    ...
    help_text = RESET_HELP[kind].replace("<id>", iid or "<id>")
    print(f"    {help_text}")
    if auto:
        return {"kind": kind, "instance": iid, "note": "reset assumed done"}
    try:
...
def wait_cleared(kind: str, deadline: float) -> bool:
    ...
    a suspiciously fast cold start.
    """
...
def wait_ready(deadline: float, seen: list[str]) -> dict:
...
def cycle(n: int, kind: str, auto: bool) -> dict:
    ...
    t0 = time.time()
    if kind != "warm":
        if not wait_cleared(kind, t0 + 300):
            row["result"] = "fail"
